...
from utility.functions import CRC
#detect_errors
...
class MainWindow(Frame):
    ...
    def detect_errors(self) -> None:
        ...
        polinomio = self.polynomial.get()
        init_vector = self.init_vector.get()
        c=CRC()
        Bol, remanente =c.encodedData(info_hex, polinomio, init_vector)
        # Mostrar el resultado
        if Bol:
            self.label_result.config(text="Resultado concuerda con la llave: " + remanente)
        else:
            self.label_result.config(text="Resultado no concuerda con la llave, hubo error en la transmisión, se esperaba: " + remanente)
